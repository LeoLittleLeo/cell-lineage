...
const WEIGHT_LABELS: Record<TaskWeight, string> = { 1: "轻量", 2: "标准", 3: "深度" };
  ...
  return (
    <div className="plan-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="plan-panel" role="dialog" aria-modal="true" aria-labelledby="plan-title">
        ...
        <span className="panel-kicker">{emergency ? "紧急形成" : "明日基因"}</span>
        <h2 id="plan-title">{emergency ? "为今天紧急形成一组基因" : "为明天设计细胞的生长"}</h2>
        <div className="plan-meta"><span>{dateLabel}</span><span>{sealed ? "已封存" : `${validTasks.length} 个事项`}</span></div>
        <p className="plan-intro">{emergency ? "昨天没有留下基因。这是一次例外形成，封存后今天只负责执行。" : "排好顺序后封存。明天的你不再编辑，只负责让这些承诺逐步显现。"}</p>
            ...
            >
              ...
              {sealed ? <span className="plan-task__weight">{WEIGHT_LABELS[task.weight]}</span> : (
                <select value={task.weight} onChange={(event) => onEdit(task.id, { weight: Number(event.target.value) as TaskWeight })} aria-label={`第 ${index + 1} 个任务的权重`}>
                  <option value={1}>轻量</option><option value={2}>标准</option><option value={3}>深度</option>
                </select>
              )}
              ...
              {sealed ? <span className="plan-task__bio-summary">{task.estimatedMinutes ?? 30} 分钟 · 精力 {task.energy ?? 3}{task.subtasks?.length ? ` · ${task.subtasks.length} 个子事项` : ""}</span> : <div className="plan-task__bio">
                <label><span>时间</span><input type="number" min={5} max={480} step={5} value={task.estimatedMinutes ?? 30} onChange={(event) => onEdit(task.id, { estimatedMinutes: Math.max(5, Number(event.target.value) || 5) })} /><small>分钟</small></label>
                <label><span>精力</span><select value={task.energy ?? 3} onChange={(event) => onEdit(task.id, { energy: Number(event.target.value) as 1 | 2 | 3 | 4 | 5 })}><option value={1}>1</option><option value={2}>2</option><option value={3}>3</option><option value={4}>4</option><option value={5}>5</option></select></label>
                <label className="plan-task__subtasks"><span>子事项 · 用逗号分隔</span><input value={(task.subtasks ?? []).map((item) => item.title).join("，")} onChange={(event) => onEdit(task.id, { subtasks: event.target.value.split(/[，,]/).map((title) => title.trim()).filter(Boolean).map((title, subIndex) => ({ id: `${task.id}_sub_${subIndex}`, title, completed: false })) })} placeholder="研究，制作，检查" /></label>
              </div>}
            </div>
          ))}
          {!tasks.length && <p className="plan-empty">明天的基因还是空白的。</p>}
        </div>
        ...
        {!sealed && confirmSeal && <div className="seal-confirm"><p>封存后，这些承诺将成为 {emergency ? "今天" : "明天"} 的基因。</p><div><button type="button" onClick={() => setConfirmSeal(false)}>再检查一次</button><button type="button" onClick={onSeal}>确认封存</button></div></div>}
        {sealed && <p className="plan-sealed-note">● 基因已封存。它将在目标日期自动成为今日细胞。</p>}
      </section>
    </div>
  );
}
